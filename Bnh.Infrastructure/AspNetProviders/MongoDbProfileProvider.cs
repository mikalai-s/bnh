    ...
    public class MongoDbProfileProvider : ProfileProvider
    {
        ...
        public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext context, SettingsPropertyCollection collection)
        {
            ...
            foreach (SettingsProperty settingsProperty in collection)
            {
                ...
                {
                    var bsonValue = bsonDocument[settingsPropertyValue.Name];
                    var value = bsonValue.RawValue;
                    // special handling of DateTime properties
                    if(settingsProperty.PropertyType == typeof(DateTime))
                        value = ((BsonDateTime)bsonValue).AsDateTime;
                    if (value != null)
                    {
        ...
        {
            var config = DependencyResolver.Current.GetService<Bnh.Core.Config>();
        ...
        public override void SetPropertyValues(SettingsContext context, SettingsPropertyValueCollection collection)
        {
            ...
            var merged = bsonDocument.Merge(mergeDocument, true);

            this.mongoCollection.Save(merged);
        }
