...
using Bnh.Infrastructure.WebSecurity.Clients;
...
namespace Bnh.Infrastructure.WebSecurity
{
    ...
    public static class OAuthWebSecurity
    {
        ...
        public static ICollection<OAuthAccount> GetAccountsFromUserName(string userName)
        {
            ...
            {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture, "WebResources.Argument_Cannot_Be_Null_Or_Empty", "userName"), "userName");
            }
        ...
        /// </returns>
        public static bool HasLocalAccount(ObjectId userId)
        {
        ...
        internal static void RegisterClient(KeyValuePair<string, IDictionary<string, string>> login)
        {
            if (login.Key == "google")
            {
                OAuthWebSecurity.RegisterClient(
                    new Google(), "Google", null);
            }
            else if (login.Key == "yahoo")
            {
                OAuthWebSecurity.RegisterClient(
                    new Yahoo(), "Yahoo", null);
            }
            ...
            else if (login.Key == "facebook")
            {
                OAuthWebSecurity.RegisterClient(
                    new Facebook(
                        login.Value["appId"],
                        login.Value["appSecret"]),
                    "Facebook", null);
            }
            else if (login.Key == "twitter")
            {
                OAuthWebSecurity.RegisterTwitterClient(
                    consumerKey: login.Value["consumerKey"],
                    consumerSecret: login.Value["consumerSecret"]);
            }
        }
